...
export default function FloatingWhyChoose() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {/* 🔴 Blinking / Pulsing Button (still animated) */}
      <motion.button
        ...
        aria-label={open ? "Close why choose" : "Open why choose"}
        title="Why choose us"
      >
        <i className={`ri-${open ? "close-line" : "question-line"} text-2xl`}></i>
      </motion.button>

      {/* 📌 Floating Card with Animation — not draggable */}
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="mt-3 w-72 sm:w-80 bg-white/60 backdrop-blur-xl rounded-2xl shadow-xl p-6 border border-white/30"
          >
            <h3 className="text-lg font-bold text-gray-800 mb-4">Why Choose Adyaraj?</h3>

            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.id} className="flex items-start gap-3">
                  <i className={`${item.icon} text-orange-600 text-xl shrink-0`}></i>
                  <div>
